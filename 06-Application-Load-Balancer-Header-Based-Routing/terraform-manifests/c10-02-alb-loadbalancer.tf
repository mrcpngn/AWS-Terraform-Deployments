module "alb" {
    ...
    https_listener_rules = [
        # Rule1: app1 host header should go to App1 EC2 Instances
        {
            ...
            conditions = [{
                # path_patterns = ["/app1*"]
                host_headers = [ format("${var.dns_prefix1}.%s","${data.aws_route53_zone.primary_domain.name}") ]
            }]
        },
        # Rule2: app2 host header should go to App2 EC2 Instances
        {
            ...
            conditions = [{
                # path_patterns = ["/app2*"]
                host_headers = [ format("${var.dns_prefix2}.%s","${data.aws_route53_zone.primary_domain.name}") ]
            }]
        },
