...
then
    ...
    sudo echo "<!DOCTYPE html> <html> <body style=\"background-color:rgb(150, 223, 232);\"> <h1>Welcome to Mads and Ollie - APP-2</h1> <p>Instance ID: ${instance_id}</p> <p>Private IP: ${private_ip}</p> <p>Application Version: V2</p> </body></html>" | sudo tee /var/www/html/app2/index.html
    curl -s http://169.254.169.254/latest/dynamic/instance-identity/document -o /var/www/html/app2/metadata.html
    ...
    TOKEN=`curl -s -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600"` && curl -s -H "X-aws-ec2-metadata-token: $TOKEN" http://169.254.169.254/latest/dynamic/instance-identity/document | sudo tee -a /var/www/html/app2/metadata.html
fi